...
function fit_FhBhDc(
    ...
    f(x_tilde::Vector{T}) where {T<:Real} = begin
        ...
        #println("r: $(ps[1]) \t\t ϵ: $(ps[2]) \t ψ: $(ps[3]) \t α: $(ps[4]) \t γ: $(ps[5])")
        println("λ: $(ps[1]+maximum([ps[2], 5*ps[4]])) \t\t μ: $(ps[2]) \t ψ: $(ps[3]) \t α: $(ps[4]) \t γ: $(ps[5])")
    ...
    dγ = Distributions.LogNormal(log(0.01), 0.2)
    ...
    dγ = Distributions.Truncated(dγ, lower[5] + tol, upper[5] - tol)
