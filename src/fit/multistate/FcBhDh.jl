function FcBhDh_newmodel(x::Vector{T}; n=6, sd=0.587) where {T<:Real}
    #α, β = x[4:5]
    ...
    dμ = Distributions.LogNormal(log(μhat), sd)

    λ = Pesto.make_quantiles2(dλ, n)
    μ = Pesto.make_quantiles2(dμ, n)
    ψ = repeat([ψ], n)

    model = FcBhDhModel(λ, μ, ψ, α, β)
...
function fit_FcBhDh(
    ...
    f(x_tilde::Vector{T}) where {T<:Real} = begin
        ...
        elseif any((x .- upper) .^ 2 .< 1e-30)
            ...
            model = FcBhDh_newmodel(x; n=n, sd=sd)
